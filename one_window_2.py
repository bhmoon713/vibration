#!/usr/bin/env python3
# Live IMU plotter with parameterized groups/layout, single-window option,
# color mapping, line styles, live FFT peak annotations, and CSV logging.
#
# Examples:
#   python3 imu_plot.py --groups acc --layout per-axis                # single window (default)
#   python3 imu_plot.py --groups acc --layout combined                # single window (combined)
#   python3 imu_plot.py --groups acc --layout per-axis --single-window 0   # old multi-window style
...
def _break_on_gaps(tt: np.ndarray, yy: np.ndarray, seq: np.ndarray, max_dt: float = 0.006):
    """Return copies of t,y with NaNs inserted at gap points so the line breaks.
       A gap is when seq isn't consecutive OR time jumps > max_dt (default 6 ms ~ 3 samples @500 Hz)."""
    if tt.size < 3:
        return tt, yy
    tt2 = tt.copy()
    yy2 = yy.copy()
    gap = np.zeros(tt.size, dtype=bool)
    gap[1:] |= (np.diff(seq) != 1)
    gap[1:] |= (np.diff(tt) > max_dt)
    if np.any(gap):
        idx = np.where(gap)[0]
        yy2[idx] = np.nan
        tt2[idx] = np.nan
...
class IMUPlotter:
    def __init__(self, port: str, baud: int, window_s: float,
        ...
        self.single_window = bool(single_window)
        ...
        # Containers for axes/lines
        self.fig = None
        self.fig_axes = {}     # per-axis layout: keyed by 'ax','ay','az','gx','gy','gz'
        self.fig_groups = {}   # combined layout: keyed by group name
    ...
    # ---------- Controls + Figures ----------
    def _controls_panel(self, ax_panel):
        ax_panel.axis("off")

        # Button areas inside the panel (axes-fraction coords)
        start_box = [0.05, 0.45, 0.40, 0.45]
        stop_box  = [0.55, 0.45, 0.40, 0.45]

        ax_start = ax_panel.inset_axes(start_box)
        ax_stop  = ax_panel.inset_axes(stop_box)

        # KEEP references on self to prevent GC
        self.btn_start = Button(ax_start, "Start Save")
        self.btn_stop  = Button(ax_stop,  "Stop Save")

        # Status text
        self.ctrl_status_txt = ax_panel.text(
            0.02, 0.10, "Status: Not saving", fontsize=10, va="bottom", ha="left"
        )
        ...
        def on_start(_event):
            ...
            ax_panel.figure.canvas.draw_idle()
    ...
    def run_ui(self):
        if self.single_window:
            if self.layout == "per-axis":
                self._single_window_per_axis()
            else:
                self._single_window_combined()
        else:
            if self.layout == "per-axis":
                self._multi_window_per_axis()
            else:
                self._multi_window_combined()

    # ---------- Multi-window (legacy) ----------
    def _multi_window_per_axis(self):
        fig_axes = {}
        for grp in self.groups:
            for ch in CHANNEL_GROUPS[grp]:
                ...
                peak_text = ax_fft.text(0, 0, "", fontsize=9, color=c, alpha=0.9, ha='left', va='bottom', visible=False)
                for ax in (ax_time, ax_fft):
                    ax.grid(True)
                ax_time.set_xlabel("Time (s)"); ax_time.set_ylabel("Signal"); ax_time.legend(loc="upper right")
                ax_fft.set_xlabel("Frequency (Hz)"); ax_fft.set_ylabel("|FFT|")
                fig_axes[ch] = {"ax_time": ax_time, "ax_fft": ax_fft,
                                "line_time": line_time, "line_fft": line_fft,
                                "peak_marker": peak_marker, "peak_text": peak_text}
        self.fig_axes = fig_axes
        self._ui_loop_per_axis()

    def _multi_window_combined(self):
        fig_groups = {}
        for grp in self.groups:
            ...
            ax_fft  = plt.subplot(2, 1, 2)
            lines_time = {
                ch: ax_time.plot([], [], label=ch, color=COLOR_MAP.get(ch, "black"),
                                 linewidth=LINE_WIDTH_TIME)[0]
                for ch in chans
            }
            fft_ch = self.fft_ch_by_group.get(grp, chans[0])
            fft_color = COLOR_MAP.get(fft_ch, "gray")
            line_fft, = ax_fft.plot([], [], color=fft_color, alpha=LINE_ALPHA_FFT, linewidth=LINE_WIDTH_FFT)
            peak_marker, = ax_fft.plot([], [], marker='o', linestyle='None', color=fft_color, alpha=0.9, visible=False)
            peak_text = ax_fft.text(0, 0, "", fontsize=9, color=fft_color, alpha=0.9, ha='left', va='bottom', visible=False)
            for ax in (ax_time, ax_fft):
                ax.grid(True)
            ax_time.set_xlabel("Time (s)"); ax_time.set_ylabel("Signal"); ax_time.legend(loc="upper right")
            ax_fft.set_xlabel("Frequency (Hz)"); ax_fft.set_ylabel("|FFT|")
            fig_groups[grp] = {"ax_time": ax_time, "ax_fft": ax_fft,
                               "lines_time": lines_time, "line_fft": line_fft,
                               "peak_marker": peak_marker, "peak_text": peak_text}
        self.fig_groups = fig_groups
        self._ui_loop_combined()

    # ---------- Single window layouts ----------
    def _single_window_per_axis(self):
        # Grid: 4 rows (3 for plots, 1 for controls) × 2 cols (Time, FFT)
        self.fig = plt.figure("IMU – Per-axis (single window)", figsize=(12, 10))
        gs = GridSpec(nrows=4, ncols=2, height_ratios=[1.0, 1.0, 1.0, 0.35], hspace=0.35, wspace=0.25, figure=self.fig)

        channels = []
        for grp in self.groups:
            channels.extend(CHANNEL_GROUPS[grp])
        # Only use first three to mirror ax/ay/az typical view
        channels = channels[:3]
        ...
        for i, ch in enumerate(channels):
            ...
            ax_time.set_title(f"{ch} Time"); ax_fft.set_title(f"{ch} FFT")
            for ax in (ax_time, ax_fft): ax.grid(True)
        ...
        # Controls spanning bottom row
        ax_panel = self.fig.add_subplot(gs[3, :])
        self._controls_panel(ax_panel)

        self.fig_axes = fig_axes
        self._ui_loop_per_axis()

    def _single_window_combined(self):
        # Grid: 3 rows (Time, FFT, Controls) × 1 col
        self.fig = plt.figure("IMU – Combined (single window)", figsize=(12, 8))
        gs = GridSpec(nrows=3, ncols=1, height_ratios=[1.0, 1.0, 0.35], hspace=0.35, figure=self.fig)

        fig_groups = {}
        for gi, grp in enumerate(self.groups):
            chans = CHANNEL_GROUPS[grp]
            # For multiple groups, we stack time axes? Simpler: single pair of axes, overlay groups
            # But user asked one main window; we overlay lines for all requested groups.
        ax_time = self.fig.add_subplot(gs[0, 0])
        ax_fft  = self.fig.add_subplot(gs[1, 0])

        lines_time = {}
        for grp in self.groups:
            for ch in CHANNEL_GROUPS[grp]:
                lines_time[ch] = ax_time.plot([], [], label=ch,
                                              color=COLOR_MAP.get(ch, "black"),
                                              linewidth=LINE_WIDTH_TIME)[0]
        # FFT: choose first group's selected channel as default when multiple groups → annotate each group separately not trivial in single axes;
        # here we compute and plot FFT for the first group/channel selected.
        first_grp = self.groups[0]
        fft_ch = self.fft_ch_by_group.get(first_grp, CHANNEL_GROUPS[first_grp][0])
        fft_color = COLOR_MAP.get(fft_ch, "gray")
        line_fft, = ax_fft.plot([], [], color=fft_color, alpha=LINE_ALPHA_FFT, linewidth=LINE_WIDTH_FFT)
        peak_marker, = ax_fft.plot([], [], marker='o', linestyle='None', color=fft_color, alpha=0.9, visible=False)
        peak_text = ax_fft.text(0, 0, "", fontsize=9, color=fft_color, alpha=0.9, ha='left', va='bottom', visible=False)

        ax_time.set_title("Time (all channels)"); ax_fft.set_title(f"FFT ({fft_ch})")
        for ax in (ax_time, ax_fft): ax.grid(True)
        ax_time.set_xlabel("Time (s)"); ax_time.set_ylabel("Signal"); ax_time.legend(loc="upper right")
        ax_fft.set_xlabel("Frequency (Hz)"); ax_fft.set_ylabel("|FFT|")

        fig_groups["combined"] = {"ax_time": ax_time, "ax_fft": ax_fft,
                                  "lines_time": lines_time, "line_fft": line_fft,
                                  "peak_marker": peak_marker, "peak_text": peak_text,
                                  "fft_group": first_grp, "fft_ch": fft_ch}

        # Controls at bottom
        ax_panel = self.fig.add_subplot(gs[2, 0])
        self._controls_panel(ax_panel)

        self.fig_groups = fig_groups
        self._ui_loop_combined(single_axes=True)

    # ---------- UI Loops (shared updating logic) ----------
    def _ui_loop_per_axis(self):
        try:
            while True:
                time.sleep(0.02)
                ...
                # Update each panel we created
                for key, axes in self.fig_axes.items():
                    y = snap[key][tmask]
                    # Time
                    y = snap[key][tmask]
                    seq = snap["seq"][tmask]
                    ttb, yb = _break_on_gaps(t_rel[tmask], y, seq)
                    axes["line_time"].set_data(ttb, yb)
    ...
    def _ui_loop_combined(self, single_axes=False):
        try:
            while True:
                time.sleep(0.02)
                t, snap = self._snapshot()
                ...
                axes = self.fig_groups["combined"]
                # Time: update all lines
                yall = []
                for ch, line in axes["lines_time"].items():
                    y = snap[ch][tmask]
                    ttb, yb = _break_on_gaps(t_rel[tmask], y, snap["seq"][tmask])
                    line.set_data(ttb, yb)
                    yall.append(y)
                axes["ax_time"].set_xlim(-self.window_s, 0)
                ...
                # FFT: compute for selected fft channel (first group's choice)
                fft_ch = axes["fft_ch"]
...
# -------------------- CLI --------------------
def parse_args():
    p = argparse.ArgumentParser(description="Live IMU plotter (parameterized, single-window capable)")
    ...
    p.add_argument("--single-window", type=int, default=1,
                   help="1=pack all plots+controls into one window (default), 0=use multiple windows")
    args = p.parse_args()
...
def main():
    ...
    print(f"[info] groups={args.groups}, layout={args.layout}, fft_map={fft_map}, single_window={bool(args.single_window)}")
